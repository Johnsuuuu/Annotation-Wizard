...
class TaskTakenPage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
        ...
        this.handleAnnotateTaskOnClick = this.handleAnnotateTaskOnClick.bind(this);
    }

    componentDidMount() {
        axios.get('/api/tasks/taken-tasks/' + this.props.auth.user.id)
            .then(res => {
                if (res.status === 200) {
                    this.setState({
                        data: res.data,
    ...
    handleAnnotateTaskOnClick(index) {
        if (this.props.auth.isAuthenticated) {
            this.props.history.push("/annotate/" + this.props.auth.user.id + "/" + this.state.data[index]._id);
        }
                                        ...
                                        <Card sx={{ maxWidth: 600 }}>
                                            ...
                                            <CardActions sx={{ pt: 0, pb: 0 }} style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                                                <ThemeProvider theme={theme}>
                                                    <IconButton disabled={task.status === "finished" ? true : false} color="primary" onClick={() => this.handleAnnotateTaskOnClick(i)}>
                                                        <Edit fontSize="small" />
                                                    </IconButton>
                                                </ThemeProvider>
                                            </CardActions>
...
export default connect(mapStateToProps)(TaskTakenPage);
